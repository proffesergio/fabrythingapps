...
let mockCartState: { lines: unknown[]; totals: { itemCount: number; subtotal: number }; loading?: boolean };
...
jest.mock('@fabrything/core', () => ({
  ...
  useCart: () => ({
    ...
    loading: mockCartState.loading ?? false,
  }),
}));
...
test('increasing quantity calls updateQuantity with quantity + 1', async () => {
  mockCartState = { lines: [line], totals: { itemCount: 2, subtotal: 1998 } };
  await renderFlushed(<Cart />);
  await pressFlushed(screen.getByLabelText('increase-11'));
...
test('decreasing quantity calls updateQuantity with quantity - 1', async () => {
  mockCartState = { lines: [line], totals: { itemCount: 2, subtotal: 1998 } };
  await renderFlushed(<Cart />);
  await pressFlushed(screen.getByLabelText('decrease-11'));
...
test('remove calls removeItem for the right variant', async () => {
  mockCartState = { lines: [line], totals: { itemCount: 2, subtotal: 1998 } };
  await renderFlushed(<Cart />);
  await pressFlushed(screen.getByText('remove'));
...
test('shows a loading view while the persisted cart is still hydrating', async () => {
  mockCartState = { lines: [], totals: { itemCount: 0, subtotal: 0 }, loading: true };
  await renderFlushed(<Cart />);
  expect(screen.queryByText('emptyCart')).toBeNull();
});
